...
OFFICIAL_MODEL_EXAMPLES: dict[str, dict[str, str]] = {
    "qwen-plus": {
        "example_python": _join_lines(
            "import json",
            "import requests",
            "",
            ...
            "        {",
            '            "role": "user",',
            '            "content": "请分析 SaaS 订阅制产品降低流失率的三个最有效策略，并给出落地建议。"',
            "        }",
            "    ],",
            '    "stream": True,',
            '    "stream_options": {',
        ...
        "example_typescript": _join_lines(
            "const response = await fetch('YOUR_BASE_URL/v1/chat/completions', {",
            "  method: 'POST',",
            "  headers: {",
            "    'Authorization': 'Bearer YOUR_API_KEY',",
            "    'Content-Type': 'application/json'",
            ...
            "    model: 'qwen-plus',",
            "    messages: [",
            "      { role: 'user', content: '请分析 SaaS 订阅制产品降低流失率的三个最有效策略，并给出落地建议。' }",
            "    ],",
            "    stream: true,",
    ...
    "deepseek-v3.2": {
        "example_python": _join_lines(
            ...
            "        {",
            '            "role": "system",',
            '            "content": "你是资深技术方案顾问，输出结构化结论。"',
            "        },",
            "        {",
            '            "role": "user",',
            '            "content": "对比自建向量检索和托管检索服务，分别从成本、上线速度、可维护性三个维度给出建议。"',
            "        }",
            "    ],",
            '    "temperature": 0.2',
            "}",
        ...
        "example_typescript": _join_lines(
            ...
            "    model: 'deepseek-v3.2',",
            "    messages: [",
            "      { role: 'system', content: '你是资深技术方案顾问，输出结构化结论。' },",
            "      { role: 'user', content: '对比自建向量检索和托管检索服务，分别从成本、上线速度、可维护性三个维度给出建议。' }",
            "    ],",
            "    temperature: 0.2",
            "  })",
            "});",
    ...
    "glm-5": {
        "example_python": _join_lines(
            ...
            "        {",
            '            "role": "system",',
            '            "content": "你是高级全栈工程师，输出可直接落地的代码方案。"',
            "        },",
            "        {",
            '            "role": "user",',
            '            "content": "请用 TypeScript 写一个带指数退避重试的 fetchJSON 函数，并补充 JSDoc 注释和一个使用示例。"',
            "        }",
            "    ],",
            '    "temperature": 0',
            "}",
        ...
        "example_typescript": _join_lines(
            ...
            "    model: 'glm-5',",
            "    messages: [",
            "      { role: 'system', content: '你是高级全栈工程师，输出可直接落地的代码方案。' },",
            "      { role: 'user', content: '请用 TypeScript 写一个带指数退避重试的 fetchJSON 函数，并补充 JSDoc 注释和一个使用示例。' }",
            "    ],",
            "    temperature: 0",
            "  })",
            "});",
    ...
    "minimax-m2.5": {
        "example_python": _join_lines(
            ...
            "        {",
            '            "role": "system",',
            '            "content": "你是企业生产力助手，擅长把复杂任务拆成可执行步骤。"',
            "        },",
            "        {",
            '            "role": "user",',
            '            "content": "为一个 SaaS 团队制定下季度 AI 落地计划，按目标、执行步骤、风险、衡量指标四部分输出。"',
            "        }",
            "    ],",
            '    "temperature": 0.3',
            "}",
        ...
        "example_typescript": _join_lines(
            ...
            "    model: 'minimax-m2.5',",
            "    messages: [",
            "      { role: 'system', content: '你是企业生产力助手，擅长把复杂任务拆成可执行步骤。' },",
            "      { role: 'user', content: '为一个 SaaS 团队制定下季度 AI 落地计划，按目标、执行步骤、风险、衡量指标四部分输出。' }",
            "    ],",
            "    temperature: 0.3",
            "  })",
            "});",
...
def get_official_model_examples(model_id: str) -> dict[str, str]:
    key = resolve_official_model_key(model_id)
    if not key:
        return {}
    return OFFICIAL_MODEL_EXAMPLES.get(key, {})
